...
#define EASY_SPDY_CTL_BIT                   1
#define EASY_SPDY_VERSION                   3

#define EASY_SPDY_DEFAULT_WINDOW_SIZE       0x10000

#define EASY_FRAME_HEAD_LENGTH              8
#define EASY_SPDY_FLAG_FIN                  0x01

#define EASY_SPDY_NV_BUF_SIZE               4096
#define EASY_SPDY_STREAM_TABLE_SIZE         32
...
#define EASY_SPDY_STREAM_OPEN               0
#define EASY_SPDY_STREAM_HALF_CLOSE         1
#define EASY_SPDY_STREAM_CLOSE              2

#define EASY_SPDY_BUFFER_SIZE               4096
//name/value header block里键值对数最大值
#define EASY_SPDY_MAX_NAME_VALUES           512
...
#define easy_spdy_ctl_frame_head(t)                                              \
    ((uint32_t) EASY_SPDY_CTL_BIT << 31 | EASY_SPDY_VERSION << 16 | (t))
...
#define easy_spdy_set_rst_stream(frame, sid, sc)                                 \
    (frame)->hd.version = EASY_SPDY_VERSION;                                 \
    (frame)->hd.flags = 0;                                                   \
    (frame)->hd.type = EASY_SPDY_RST_STREAM;                                 \
    (frame)->hd.length = 8;                                                  \
    (frame)->stream_id = (sid);                                              \
...
#define easy_spdy_set_goaway(frame, sid, sc)                                     \
    (frame)->hd.version = EASY_SPDY_VERSION;                                 \
    (frame)->hd.flags = 0;                                                   \
    (frame)->hd.type = EASY_SPDY_GOAWAY;                                     \
    (frame)->hd.length = 8;                                                  \
...
struct easy_spdy_stream_t {
    ...
    z_stream                *zst_in;
#ifdef HAVE_BROTLI
    BrotliState             *brotli_state;
#endif
    char                    buffer[EASY_SPDY_BUFFER_SIZE];
};

typedef enum {
    EASY_SPDY_SETTINGS_UPLOAD_BANDWIDTH =               1,
    EASY_SPDY_SETTINGS_DOWNLOAD_BANDWIDTH =             2,
    EASY_SPDY_SETTINGS_ROUND_TRIP_TIME =                3,
    EASY_SPDY_SETTINGS_MAX_CONCURRENT_STREAMS =         4,
    EASY_SPDY_SETTINGS_CURRENT_CWND =                   5,
    EASY_SPDY_SETTINGS_DOWNLOAD_RETRANS_RATE =          6,
    EASY_SPDY_SETTINGS_INITIAL_WINDOW_SIZE =            7,
    EASY_SPDY_SETTINGS_CLIENT_CERTIFICATE_VECTOR_SIZE = 8,
} easy_spdy_settings_id_t;

typedef enum {
    EASY_SPDY_ID_FLAG_SETTINGS_NONE =                     0,
    EASY_SPDY_ID_FLAG_SETTINGS_PERSIST_VALUE =            1,
    EASY_SPDY_ID_FLAG_SETTINGS_PERSISTED =                2
} easy_spdy_settings_flags_t;

typedef enum {
    EASY_SPDY_SYN_STREAM =     1,
    EASY_SPDY_SYN_REPLY =      2,
    EASY_SPDY_RST_STREAM =     3,
    EASY_SPDY_SETTINGS =       4,
    EASY_SPDY_NOOP =           5,
    EASY_SPDY_PING =           6,
    EASY_SPDY_GOAWAY =         7,
    EASY_SPDY_HEADERS =        8,
    EASY_SPDY_WINDOW_UPDATE =  9,
    EASY_SPDY_CREDENTIAL =     10
} easy_spdy_ctrl_frame_type_t;

typedef enum {
    EASY_SPDY_HTTP_DATA          = 0,
    EASY_SPDY_FRAME_CTL          = 1,
    EASY_SPDY_FRAME_DATA         = 2,
    EASY_SPDY_FRAME_INTERNAL_CTL = 3,
    EASY_SPDY_FRAME_PING         = 4,
    EASY_SPDY_FRAME_RST_STREAM   = 5,
