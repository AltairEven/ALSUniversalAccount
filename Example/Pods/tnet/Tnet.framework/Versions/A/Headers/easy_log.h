...
#define easy_fatal_log(format, args...) if(easy_log_level>=EASY_LOG_FATAL)      \
        easy_log_format(EASY_LOG_FATAL, NULL, __LINE__, __FUNCTION__, format, ## args)
#define easy_error_log(format, args...) if(easy_log_level>=EASY_LOG_ERROR)      \
        easy_log_format(EASY_LOG_ERROR, NULL, __LINE__, __FUNCTION__, format, ## args)
#define easy_warn_log(format, args...) if(easy_log_level>=EASY_LOG_WARN)        \
        easy_log_format(EASY_LOG_WARN, NULL, __LINE__, __FUNCTION__, format, ## args)
#define easy_info_log(format, args...) if(easy_log_level>=EASY_LOG_INFO)        \
        easy_log_format(EASY_LOG_INFO, NULL, __LINE__, __FUNCTION__, format, ## args)
#if defined(DEBUG_LOG)
#define easy_debug_log(format, args...) if(easy_log_level>=EASY_LOG_DEBUG)      \
        easy_log_format(EASY_LOG_DEBUG, NULL, __LINE__, __FUNCTION__, format, ## args)
#define easy_trace_log(format, args...) if(easy_log_level>=EASY_LOG_TRACE)      \
        easy_log_format(EASY_LOG_TRACE, NULL, __LINE__, __FUNCTION__, format, ## args)
#else
...
#define easy_log_common(format, args...) do{}while(0)
#define easy_log_init() do{}while(0)
